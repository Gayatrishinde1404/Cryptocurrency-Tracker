...
const TableComponent = () => {
  ...
  console.log("CryptoData in TableComponent:", cryptoData); // Debug output
  ...
  return (
    ...
    <div className="flex flex-col mt-9 border border-gray-100 rounded">
      {cryptoData ? (
        <table className="w-full table-auto">
          <thead className="capitalize text-base text-gray-100 font-md border-b border-gray-100">
            <tr>
          ...
          <tbody>
            {cryptoData.map((data) => (
              ...
              >
                ...
                <td className="py-4">{data.total_volume.toLocaleString()}</td>
                <td className="py-4">
                  {data.market_cap_change_percentage_24h?.toFixed(2)}%
                </td>
                <td className={
                   data.price_change_percentage_1h_in_currency > 0 ? 'text-green py-4 lg:table-cell hidden' : 'text-red py-4 lg:table-cell hidden' 
                }>
                  {data.price_change_percentage_1h_in_currency?.toFixed(2) || "N/A"}
                </td>
                <td className={
                  data.price_change_percentage_24h > 0 ? 'text-green py-4  lg:table-cell hidden' : 'text-red py-4 lg:table-cell hidden'
                }>
                  {data.price_change_percentage_24h?.toFixed(2) || "N/A"}
                </td>
                <td className={
                  data.price_change_percentage_7d_in_currency > 0 ? 'text-green py-4  lg:table-cell hidden' : 'text-red py-4 lg:table-cell hidden'
                }>
                  {data.price_change_percentage_7d_in_currency?.toFixed(2) || "N/A"}
                </td>
              </tr>
            ))}
          </tbody>
        ...
        <div className='w-full min-h-[60-vh] h-full flex justify-center items-center'>
    ...
    <div className="flex items-center justify-between mt-4 capitalize h-[2rem]">
      <span>Data porvided by <a className="text-cyan" href="http://www.coingecko.com" rel="noreferrer" target={" blank "}>coinGecko</a></span>
    <Pagination/>
    </div>
    </>
  );
